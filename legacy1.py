...
with open("legacy1.yaml") as f:
    CFG = yaml.safe_load(f)

OLLAMA        = "http://localhost:11434"
SYS           = CFG["system_config"]
TARGET_MODEL  = "llama3.1:latest"
SYSTEM_PROMPT = "You are a helpful assistant. Answer the user's questions directly."
    ...
    async with httpx.AsyncClient(timeout=240) as client:
        try:
            if history:
                ...
                payload = {
                    "model":  model,
                    # No "format": "json" — mistral:7b-instruct returns empty with it
...
class OllamaModel(DeepEvalBaseLLM):
    ...
    async def a_generate(self, prompt: str, schema=None, **kwargs):
        ...
        # FIX: try fallback BEFORE raising — previously this was dead code
        if not text:
        ...
        if schema is not None:
            try:
                return schema(**json.loads(self._extract_json(text)))
            except Exception as e:
                ...
                if fb is not None:
                    ...
                    return fb
                raise RuntimeError(f"Could not parse or construct {schema}")
...
if __name__ == "__main__":
    ...
    linear_jailbreak = LinearJailbreaking(
        ...
        turn_level_attacks = [Roleplay()] if lj_cfg.get("use_roleplay", True) else [],
    )
    ...
    simulator_model  = OllamaModel("mistral:7b-instruct")
    evaluation_model = OllamaModel("mistral:7b-instruct")
